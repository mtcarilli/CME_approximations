...
class my_MLP1(nn.Module):
	...
	if not self.fit_hyp:
		...
		out = self.output(l_4)
		w_pred = self.softmax(out[:-1])
		hyp_pred = self.sigmoid(out[-1])*5+1
		return w_pred,hyp_pred
...
#    model.eval()
    ...
    w_p1 = model_out[0][0]
    hyp = model_out[1]
    predicted_y1 = get_ypred_at_RT(p1[0],npdf,w_p1,hyp=hyp)
...
def train_MLP1(train_list,test_list,num_epochs,npdf,batchsize,get_ypred_at_RT,learning_rate=1e-3,
    ...
    model = my_MLP1(3,npdf)      # define model 
