...
class my_MLP1(nn.Module):
    ...
    def __init__(self, input_dim, output_dim,fit_hyp=False):
        super().__init__()
        ...
        self.fit_hyp = fit_hyp
        if fit_hyp:
            self.output = nn.Linear(128, output_dim+1)
    ...
    def forward(self, parameters):
        ...
        if self.fit_hyp:
            out = self.output(l_4)
            fwd_out = torch.empty(size=out.size())
            fwd_out[:-1] = self.softmax(out[:-1])
            fwd_out[-1] = self.sigmoid(out[-1])*5+1 #the bounds should be defined in __init__ instead of hardcoded
            return fwd_out
        else:
...
def loss_fn(p_list,y_list,w,npdf,batchsize,get_ypred_at_RT,metric='kld',fit_hyp=False):
    ...
    for b in range(batchsize):
        ...
        w_ = w[b]
        if fit_hyp:
            Y_ = get_ypred_at_RT(p,npdf,w_[:,:-1],hyp=w_[:,-1])
        else:
...
#    model.eval()
    ...
    model_out = model.forward(p1)
    w_p1 = model_out[0]
    if model.fit_hyp:
        predicted_y1 = get_ypred_at_RT(p1[0],npdf,w_p1[:-1],hyp=w_p1[-1])
    else:
...
def train(parameters_tensor,y_list,npdf,model,optimizer,batchsize,get_ypred_at_RT,metric = 'kld'):
    ...
    for j in range(trials):
        ...
        # Compute loss
        loss = loss_fn(p_list=p,y_list=y,w=w_pred,npdf=npdf,batchsize=batchsize,get_ypred_at_RT =get_ypred_at_RT, metric=metric,fit_hyp=model.fit_hyp)
...
def train_MLP1(train_list,test_list,num_epochs,npdf,batchsize,get_ypred_at_RT,learning_rate=1e-3,
                     metric='kld',fit_hyp=False):
    

    model = my_MLP1(3,npdf,fit_hyp)      # define model 
